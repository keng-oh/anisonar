...
# docker build -t anisonar .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name anisonar anisonar
...
# postgresql-client-16: pg_dump はサーバー（postgres:16）と同じメジャーバージョン以上が必要なため、
